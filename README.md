...
#### Introduction
- 데이터를 단순히 다루는 것을 넘어, **근거를 끝까지 추적하는 집요함**이 있습니다.
  - "왜 이 변수가 중요한가", "왜 이 모델이 더 잘 작동하는가"를 설명하지 못하면 분석이 완성됐다고 생각하지 않습니다. 산업데이터공학과에서 배운 통계적 사고를 바탕으로, 직관보다 수치와 근거를 먼저 찾습니다.

- **추상적인 인사이트보다 구체적이고 검증 가능한 결과**를 만들려고 노력합니다.
  - "성능이 향상됐다"가 아니라 "정확도가 87%에서 92%로 올랐고, 그 원인은 피처 X의 전처리 방식 변경이었다"처럼 재현 가능한 분석을 지향합니다.

- 분석 결과를 **누구나 이해할 수 있는 언어로 전달**하는 것을 중요하게 생각합니다.
  - KT 에이블스쿨 프로젝트 발표와 블로그 기록을 통해, 기술적인 내용을 비전공자도 납득할 수 있도록 시각화하고 설명하는 연습을 하고 있습니다.

- 문제를 풀기 전에 **문제를 올바르게 정의하는 것**에 집중합니다.
